...
function SinglePost() {
    ...
    const fetchData = async () => {
      try {
        ...
        const commentRes = await axios.get(
          `http://blog-website-t32p.vercel.app/api/comments/${id}`
        );
  ...
  const handleLike = async () => {
    ...
    try {
      const res = await axios.put(
        `http://blog-website-t32p.vercel.app/api/posts/like/${id}`,
        {},
        {
          headers: {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://blog-website-t32p.vercel.app/api/comments",
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      await axios.delete(
        `http://blog-website-t32p.vercel.app/api/comments/${commentId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
...
const handleBookmark = async (postId) => {
  ...
  try {
    const res = await axios.put(
      "http://blog-website-t32p.vercel.app/api/auth/bookmark",
      { postId },
      {
        headers: {
          Authorization: `Bearer ${token}`,
